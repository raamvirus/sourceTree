...
object LogAnalyzer {
	def main(args: Array[String]) {
		val conf = new SparkConf()
						.setMaster("local[2]")
						.setAppName("Server Log Analyzer")
						.setAppName("spark.executor.memory","1g")	
		val sc = new SparkContext(conf)
		
		val logFile = "file:///home/user/server_log"
		...
		*/
		println("Total msgs: %s, MySQL errs: %s, PHP errs: %s, Rails: %s, Warnings: %s".format(tot, msql, php, rail, (tot - (msql+php+rail))))
	}
}
